...
class Testing(gr.top_block, Qt.QWidget):
    ...
    def handle_load_mission(self):
        # Logic to load a mission
        file_path, _ = Qt.QFileDialog.getOpenFileName(self, "Load Mission", "", "Mission Files (*.mission)")
        if file_path:
            self.load_mission(file_path)

    def handle_save_mission(self):
        # Logic to save a mission
        file_path, _ = Qt.QFileDialog.getSaveFileName(self, "Save Mission", "", "Mission Files (*.mission)")
        if file_path:
            self.save_mission(file_path)
